...
class Selector:
    ...
    def selectAll(self):
        ...
        if vars.debug: print len(patients_list)    
        for patient in patients_list:
            ...
            g=self.constructEEGGraph(dgs)#EEG Graph Object
            data.append((file_name,g))
            f.close()
        data=self.filter_against_opotometry_table(data)
        print "Number of all data: %d" %len(data)
        return data
    ...
    def filter_against_opotometry_table(self,data):
        ...
        for filename,g in data:
            ...
            #cv=OPTOMETRY[OPTOMETRY.ID==id][OPTOMETRY.eye==vars.geye]  
        ...
        if vars.debug: print "clinical_variables is in shape (%s,%s)" %(clinical_variables.shape[0],clinical_variables.shape[1])
